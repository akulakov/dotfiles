...
" Does not work right.. eats one <cr> and deletes one '>'
...
fun s:Return()
	...
	let x = (tag != '' && match(getline('.'), '</'.tag.'>'))
	if x > -1
		return "\<cr>\<cr>\<up>"
	elseif
		return "\<cr>"
	endif
	" return tag != '' && match(getline('.'), '</'.tag.'>') > -1 ? "\<cr>\<cr>\<cr>\<up>":"\<cr>"
endf
...
fun s:CloseTag()
	...
	if line[col] !~ '\w\|<\|>' && !s:InComment()
		...
		if tag != '' && tag !~ '\vimg|input|link|meta|br|hr|area|base|param|dd|dt'
					" \ && !s:ClosingTag(tag)
			" AK 3.15.12 - comment out above line, ClosingTag() is buggy
